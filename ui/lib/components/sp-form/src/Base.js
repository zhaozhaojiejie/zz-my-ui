 ...
 * @module $ui/components/sp-form/src/Base
 */
 ...
 * 插槽
 * @member slots
 * @property {string} before 输入组件前面的内容，仅当父组件是spForm有效
 * @property {string} after 输入组件后面的内容，仅当父组件是spForm有效
 * @property {string} label 定义字段的label内容，仅当父组件是spForm有效
 * @property {string} error 作用域插槽，定义验证错误提示内容，仅当父组件是spForm有效
 */
...
export default {
   ...
   * @property {string} [width] 宽度，css属性，支持像素，百分比和表达式，也可以在spForm中统一设置itemWidth
  ...
  computed: {
    // 如果有name参数，并且是spForm的子组件，即与spForm的currentModel作双向绑定
    // 否则与组件自身的value作双向绑定
    fieldValue: {
      get() {
  ...
  methods: {
    ...
    renderComponent(vnode) {
      ...
      const scopedSlots = this.$scopedSlots.error
        ? {
          error: props => (
            <div class="el-form-item__error sp-from__custom-error">
