  ...
  <div :class="classes">
    <template v-if="tag">
      ...
      <Tag v-for="(item,index) in optionsComputed"
           :key="index"
           type="primary"
           :class="{'sp-tag-select__tag':true, 'is-disabled':item.disabled, 'is-checked':isChecked(item)}"
           :size="size"
    ...
    <template v-else>
      <span v-if="multiple && checkAll" :class="{'sp-tag-select__tag':true, 'is-checked':isAll}"
    ...
    <a v-if="collapsible" class="sp-tag-select__expand" @click="toggleCollapse">
...
<script>
  /**
   * 标签选择组件
   * @module $ui/components/sp-form/src/common/TagSelect
   */
  ...
  export default {
    ...
    computed: {
      ...
      classes() {
        return {
          'sp-tag-select': true,
          [`sp-tag-select--${this.size}`]: !!this.size,
          'sp-tag-select--text': !this.tag,
