<template>
  <div class="sp-resize" :class="classes" :style="styles">
    <slot></slot>
    <div v-if="helper && resizing" class="sp-resize__helper" :style="helperStyle"></div>
    <sp-drag v-if="!disabled && (!axis || axis==='h')"
             ref="ctrlH"
             class="sp-resize__ctrl-h"
             axis="h"
             :origin="getOrigin"
             @start="handleStart"
             @drag="handleDragH"
    ...
    <sp-drag v-if="!disabled && (!axis || axis==='v')"
             ref="ctrlV"
             class="sp-resize__ctrl-v"
             axis="v"
             :origin="getOrigin"
             @start="handleStart"
             @drag="handleDragV"
    ...
    <sp-drag v-if="!disabled && !axis"
             ref="ctrl"
             class="sp-resize__ctrl"
             :origin="getOrigin"
             @start="handleStart"
             @drag="handleDrag"
             @stop="handleStop">
...
<script>
  ...
  /**
   * spResize 拖拽更新尺寸组件, 与drag同时使用时，drag要在外层
   * @module $ui/components/sp-resize
   *
   */
  ...
  export default {
    ...
    methods: {
      ...
      handleStart(vm) {
         ...
         * 开始拖拽时触发
         * @event start
         * @param {VueComponent} resize spResize实例
         */
