<template>
  <el-card class="sp-login"
           v-bind="$attrs"
           :style="styles"
           :class="classes">
    <div v-if="logo" class="sp-login__logo">
      <slot name="logo">
        <img :src="logo" alt="">
      </slot>
    </div>
    <div v-if="title" class="sp-login__title">
      <slot name="title">{{title}}</slot>
    </div>
    <sp-form class="sp-login__form"
             ref="form"
             :rules="rules"
             @submit="handleSubmit"
             :footer="null">
      <sp-input :props="{prefixIcon:'el-icon-user'}"
                class="sp-login__input"
                :disabled="loading"
                :name="keyMap.account"
                :placeholder="accountPlaceholder"></sp-input>
      <sp-input :props="{prefixIcon:'el-icon-lock',type:'password'}"
                class="sp-login__input"
                :disabled="loading"
                :name="keyMap.password"
                :placeholder="passwordPlaceholder"></sp-input>
      <div v-if="$slots.extra" class="sp-login__extra">
        <slot name="extra"></slot>
      </div>
    </sp-form>

    <el-row class="sp-login__action" :gutter="10">
      <el-col :span="actionSpan">
        <el-button type="primary" :loading="loading" @click="submit">登录</el-button>
      </el-col>
      <el-col v-if="pki" :span="actionSpan">
        <el-button type="success" class="sp-login__pki" @click="handlePki">PKI</el-button>
      </el-col>
    </el-row>
