<script>
  /**
   *  图标工具按钮, 只能用在 sp-navbar 的 actions 插槽
   *  @module $ui/components/sp-navbar-IconAction
   */
  ...
  export default {
    ...
    methods: {
      iconRender() {
        return (
          <sp-icon class="sp-navbar-icon-action__icon" name={this.icon} svg={this.svg}></sp-icon>
        )
      },
      wrapTooltip(vnode) {
    ...
    render() {
      ...
      return (
        <span class="sp-navbar-icon-action" {...{on: this.$listeners}}>{vnode}</span>
      )
    }
  }
</script>
