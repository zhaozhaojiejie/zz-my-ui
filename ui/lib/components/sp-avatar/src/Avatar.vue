<template>
  <Tooltip v-bind="tooltipProps" :disabled="!tooltipProps">
    <span :class="classes" :style="styles" v-on="$listeners">
      <sp-icon :style="iconStyle" v-if="icon" class="sp-avatar__icon" v-bind="iconProps"></sp-icon>
      <img v-if="src" :src="src" :alt="alt" @error="onError">
      <span v-if="$slots.default" ref="text" class="sp-avatar__string" :style="transformStyle"><slot></slot></span>
    </span>
  </Tooltip>
</template>
...
<script>
  /**
   *  头像组件
   *  @module $ui/components/sp-avatar
   */
  ...
  export default {
    ...
    computed: {
      ...
      classes() {
        return {
          'sp-avatar': true,
          'sp-avatar--icon': !!this.icon,
          'sp-avatar--text': !!this.$slots.default,
          'sp-avatar--image': !!this.src,
