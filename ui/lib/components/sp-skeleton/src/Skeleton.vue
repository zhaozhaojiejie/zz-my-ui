<template>
  <div :class="classes">
    <template v-if="loading">
      <div class="sp-skeleton__header" v-if="avatar">
        <span class="sp-skeleton__avatar" :class="avatarClasses"></span>
      </div>
      <div class="sp-skeleton__content" v-if="title || paragraph">
        <h3 v-if="title" class="sp-skeleton__title" :style="titleStyle"></h3>
        <ul v-if="paragraph" class="sp-skeleton__paragraph">
...
<script>
  /**
   * 骨架屏效果
   * @module $ui/components/sp-skeleton
   */
    ...
    computed: {
      classes() {
        return ['sp-skeleton', {
          ...
          'sp-skeleton--with-avatar': !!this.avatar
        }]
      },
      avatarClasses() {
