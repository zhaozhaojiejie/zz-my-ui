<script>
  /**
   *  描述列表组件
   *  @module $ui/components/sp-detail
   */
  ...
  export default {
    ...
    computed: {
      classes() {
        return ['sp-detail', `is-${this.size || 'default'}`, `is-${this.layout}`, {
    ...
    methods: {
      ...
      renderRow(children, index) {
        ...
        if (this.layout === 'vertical') {
          return ([
            <tr class="sp-detail__row" key={`label-${index}`}>
              {cloneChildren}
            </tr>,
            <tr class="sp-detail__row" key={`content-${index}`}>
              {cloneContentChildren}
            </tr>
          ])
        }
        return (
          <tr class="sp-detail__row" key={index}>
            {cloneChildren}
          </tr>
        )
      },
    ...
    render() {
      ...
      const titleVNode = title ? <div class="sp-detail__title">{title}</div> : null
      return (
        <div class={this.classes}>
          {titleVNode}
          <div class="sp-detail__view">
            <table>
              <tbody>
              {
