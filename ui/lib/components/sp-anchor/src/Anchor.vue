<template>
  <spAffix v-bind="affix" @viewUpdate="handleViewUpdate">
    <div class="sp-anchor__wrapper">
      <div class="sp-anchor">
        <div class="sp-anchor__ink">
          <span class="sp-anchor__ball" :class="ballClass" :style="ballStyle"></span>
        </div>
        <spAnchorLink v-for="(item, index) in data"
                      v-bind="item"
                      :key="index"
                      :active="activeLink"
                      @click="handleClick">
...
<script>
   ...
   * @author chenhuachun
   *
   * @module $ui/components/sp-anchor
   *
   */
  import {spAffix} from '$ui'
  import spAnchorLink from './Link'
  ...
  export default {
     ...
     * 属性参数
     * @member props
     * @property {Object} [affix] spAffix组件配置项 {offsetTop, offsetBottom, target}
