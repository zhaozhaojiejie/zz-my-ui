<template>
  <div :class="classes">
    <div v-if="iconProps || exceptionIcon|| $slots.icon"
      ...
      <slot name="icon">
        <sp-icon v-if="iconProps" class="sp-result__icon" v-bind="iconProps"></sp-icon>
    ...
    <div v-if="$slots.default" class="sp-result__content">
      <slot></slot>
    </div>
...
<script>
  /**
   * 结果展示组件
   * @module $ui/components/sp-result
   */
  ...
  export default {
    ...
    computed: {
      classes() {
        return {
          'sp-result': true,
          [`sp-result--${this.status}`]: !!this.status
        }
      },
      iconProps() {
