<template>
  <div class="sp-sidebar" :class="classes">
    <!-- 品牌区 -->
    <div class="sp-sidebar__brand" v-if="logo || title">
      <a v-if="collapsed" :href="href">
        <slot name="brand" :title="title" :logo="logo">
          <img class="sp-sidebar__logo" v-if="logo" :src="logo" :alt="title">
        </slot>
      </a>
      <a v-else :href="href">
        <slot name="brand" :title="title" :logo="logo">
          <img class="sp-sidebar__logo" v-if="logo" :src="logo" :alt="title">
          <h1 class="sp-sidebar__title" v-if="title">{{title}}</h1>
        </slot>
        <span v-if="version" class="sp-sidebar__version">{{version}}</span>
      </a>
    </div>
    <div v-if="collapsible" @click="toggleCollapse" class="sp-sidebar__trigger">
      <sp-icon :name="triggerIcon" svg></sp-icon>
    </div>
    <div class="sp-sidebar__body" :class="bodyClasses">
      <!-- 前置扩展内容 -->
      <slot></slot>
...
<script>
  /**
   * 侧边导航
   * @module $ui/components/sp-sidebar
   */

  import {spMenu, spIcon} from '$ui'
