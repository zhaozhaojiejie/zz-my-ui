<script>
  /**
   * 布局组件，围绕着主体内容4个方向排版
   * @module $ui/components/sp-layout
   */
  ...
  export default {
    ...
    methods: {
      ...
      regionRender(type) {
        ...
        }, (this.fit && size) ? [<div class="sp-layout__fit">{this.$slots[type]}</div>] : this.$slots[type])
      }
    },
    render() {
      const {north, south, west, east} = this.$slots
      ...
      const wrapHorizontal = () => {
        if (west || east) {
          return (
            <Container class="sp-layout__wrapper" direction="horizontal">
              {
                [
      ...
      const wrapVertical = () => {
        if (north || south) {
          return (
            <Container class="sp-layout__wrapper" direction="vertical">
              {
                [
