<template>
  <spLayout class="sp-crud" :north="{height:'auto'}" split :fit="fit">
    <template v-if="$slots.filter" v-slot:north>
      <spContainer>
        <spFilter v-bind="filter"
                  :model="currentQuery"
    ...
    <!-- 详情 -->
    <spDialog v-if="currentRow && viewDialog!==null"
    ...
    <!-- 新增、编辑表单 -->
    <spDialog class="sp-crud-form-dialog"
              ...
              target="body"
              :title="isEdit ? '编辑': '新增'">
      <spForm v-bind="formProps" :model="currentRow" :on-submit="handleFormSubmit">
...
<script>
  /**
   *  增删查改 组件
   *  @module $ui/components/sp-crud
   */

  import {spLayout, spContainer, spFilter, spTable, spDialog, spForm} from '$ui'
  ...
  export default {
     ...
     * @property {Object} [filter] 筛选组件 spFilter 配置参数
     * @property {Object} [query] 初始查询参数对象
     * @property {Object} [tableOptions] 表格组件 spTable 配置参数
