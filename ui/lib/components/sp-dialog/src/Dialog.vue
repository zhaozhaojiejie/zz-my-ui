<template>
  <transition :name="animation">
    <div v-if="currentVisible" v-show="!currentMinimized" class="sp-dialog__wrapper" :style="wrapperStyle">
      <div class="sp-dialog__modal" v-if="modal"></div>
      <spDrag ref="dialog"
        ...
        <spResize ref="resize"
          ...
          <Panel ref="panel"
            ...
            <spSpin fit :tip="loadingTip" :loading="loading">
...
<script>
  /**
   * 弹窗组件
   * @module $ui/components/sp-dialog
   */
  import {spDrag, spResize, spSpin} from '$ui'
  ...
  export default {
     ...
     * @property {boolean|object} [draggable] 拖拽配置，参考spDrag组件
     * @property {boolean|object} [resizable] resize配置，参考spResize组件
    ...
    methods: {
      ...
      setBodyHidden(hidden) {
        const name = 'sp-dialog-hidden-' + this._uid
