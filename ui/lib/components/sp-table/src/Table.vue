<template>
  <div class="sp-table" :class="classes">
    <spSpin v-bind="spin" fit :loading="isPager && currentLoading">
      <Toolbar v-if="toolbar" ref="toolbar" :title="title" :size="$attrs.size">
        <slot name="title" slot="title"></slot>
        <slot name="actions" slot="actions"></slot>
      </Toolbar>
      <div class="sp-table__wrapper" :style="{height:tableHeight}">
        ...
        <ElTable ref="elTable"
          ...
          <template v-if="rendered">
            ...
            <TableColumn v-if="columnFilter" :resizable="false" width="24px" class-name="sp-table--not-drag"
                         fixed="right">
          ...
          <template v-slot:append>
            ...
            <!-- 滚动加载loading 插槽-->
            <div v-if="isScroll && currentLoading" class="sp-table__skeleton">
              <slot name="skeleton">加载中...</slot>
            </div>

            <div v-if="isScroll && !currentLoading && isNoMore" class="sp-table__complete">
              <!-- 滚动加载完成提示 插槽-->
...
<script>
  ...
  export default {
     ...
     * @property {Object} [spin] 加载中的spSpin组件配置参数
    ...
    computed: {
      classes() {
        return {
          'is-fit': this.fit,
          [`sp-table--${this.mode}`]: !!this.mode,
          'sp-table--header-bg': this.headerBackground,
