...
<script>
  ...
  export default {
     ...
     * 属性参数
     * @member props
     * @property {String|Array} [accept] 可放置的拖拽实例名称，与spDrag的group参数值对应
     ...
     * @property {Boolean} [prevent] 是否阻止放置事件传播, sp-drop 允许嵌套，为true时，子组件触发drop，不会传播给父组件
     */
    props: {
    ...
    computed: {
      ...
      classes() {
        return {
          ...
          'is-enter': this.enter,
          'sp-drop--active-highlight': (this.active && this.activeHighlight),
          'sp-drop--enter-highlight': (this.enter && this.enterHighlight),
    ...
    methods: {
      ...
      bindEventBus() {
        bus.$on('sp-drag:start', this.handleDragStart)
        bus.$on('sp-drag:dragging', this.handleDragDragging)
        bus.$on('sp-drag:stop', this.handleDragStop)
      },
      unbindEventBus() {
        bus.$off('sp-drag:start', this.handleDragStart)
        bus.$off('sp-drag:dragging', this.handleDragDragging)
        bus.$off('sp-drag:stop', this.handleDragStop)
      },
      getParent() {
        let parent = this.$parent
        while (parent && parent.$options.name !== 'spDrop') {
