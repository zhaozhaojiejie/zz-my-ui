<template>
  <div class="sp-list" :class="classes">
    <!--头部-->
    <div v-if="$slots.header"
         ref="header"
         class="sp-list__header"
         :style="headerStyle">
    ...
    <!-- 列表内容 -->
    <div ref="viewport"
         class="sp-list__wrapper"
         @scroll="handleVirtualScroll"
         :class="wrapperClass"
         :style="wrapperStyle">
      <!-- 内容 -->
      <spSpin v-bind="spin" :fit="isFit" :loading="(isPager || isVirtual) && currentLoading">

        <!-- 无数据 -->
        <div v-if="!list.length && !errorMessage" class="sp-list__empty">
          <slot name="empty">
            暂无数据
          </slot>
        </div>

        <div v-if="error && errorMessage" class="sp-list__error">
        ...
        <div v-if="isVirtual" class="sp-list__placeholder" :style="{height:`${contentHeight}px`}"></div>

        <ul ref="list"
            class="sp-list__content"
        ...
        <!-- 骨架 -->
        <div v-if="isScroll && !isNoMore && currentLoading" class="sp-list__scroll-loading">
          <slot name="skeleton">
        ...
        <!-- 没有更多提示 -->
        <div v-if="isScroll && isNoMore && currentPage>1" class="sp-list__complete">
          <slot name="complete">
            没有更多了
          </slot>
        </div>
...
<script>
  ...
  export default {
     ...
     * @property {Object} [spin] 分页模式的加载中组件配置 {tip, size}，参考spSpin组件
