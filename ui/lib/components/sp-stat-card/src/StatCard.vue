<template>
  <Card class="sp-stat-card" v-bind="$attrs" :body-style="bodyStyle">
    <div class="sp-stat-card__actions" :style="actionsStyle">
    ...
    <div v-if="icon" class="sp-stat-card__icon" :class="[`is-${iconType}`]">
      <spIcon v-bind="iconProps"></spIcon>
    </div>
    <div class="sp-stat-card__wrapper" :class="{'is-has-icon':!!icon}">
      <div v-if="title || $slots.title" class="sp-stat-card__title">
        <slot name="title">{{title}}</slot>
      </div>
      <div class="sp-stat-card__total">
        <slot name="total">{{total}}</slot>
      </div>
      <div v-if="$slots.default" class="sp-stat-card__content">
        <slot></slot>
      </div>
    </div>
    <div v-if="$slots.footer" class="sp-stat-card__footer">
...
<script>
  /**
   * 统计卡片
   * @module $ui/components/sp-stat-card
   */
