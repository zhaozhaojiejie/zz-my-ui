<template>
  <div class="sp-wrapper" :class="{'is-fit': fit,'is-split':split,'is-no-header':!header}">
    <div v-if="header" class="sp-wrapper__header" :class="classes">
      <sp-breadcrumb class="sp-wrapper__breadcrumb"
                     v-if="breadcrumb && breadcrumb.length"
                     :data="breadcrumb"></sp-breadcrumb>
      <sp-float v-if="$slots.title || $slots.actions || title"
                class="sp-wrapper__ft">
        <sp-float-item float="left"
                       class="sp-wrapper__title">
          <span v-if="back" @click="handleBack" class="sp-wrapper__back"><i class="el-icon-back"></i></span>
          <slot name="title">{{title}}</slot>
        </sp-float-item>
        <sp-float-item float="left" class="sp-wrapper__sub-title">
          <slot name="subTitle">{{subTitle}}</slot>
        </sp-float-item>
        <sp-float-item v-if="$slots.actions"
                       class="sp-wrapper__actions"
                       float="right">
          <slot name="actions"></slot>
        </sp-float-item>
      </sp-float>

      <div v-if="$slots.extra || extra" class="sp-wrapper__extra">
        <slot name="extra">{{extra}}</slot>
      </div>

      <div v-if="links && links.length" class="sp-wrapper__links">
        <span v-for="(item, index) in links"
              :key="`link${index}`"
              class="sp-wrapper__links-item"
    ...
    <div ref="body" class="sp-wrapper__body">
      <slot></slot>
    </div>

    <div v-if="$slots.footer" class="sp-wrapper__footer">
...
<script>
  /**
   * 页面容器组件
   * @module $ui/components/sp-wrapper
   */
  import {spBreadcrumb, spFloat, spFloatItem} from '$ui'
