...
function SetAlpha(c, a) {
  ...
  if (c[0] === '#') {
    if (!hexlookup3[c]) {
 if (c.length === 4) { hexlookup3[c] = 'rgba(' + hexlookup1[c[1]] + hexlookup1[c[2]] + hexlookup1[c[3]]; } else { hexlookup3[c] = 'rgba(' + hexlookup2[c.substr(1, 2)] + hexlookup2[c.substr(3, 2)] + hexlookup2[c.substr(5, 2)]; } 
}
    d = hexlookup3[c] + ae;
...
Oproto.DrawColourImage = function (c, x, y, w, h, colour, tag, x1, y1) {
  var ccanvas = c.canvas, fx = ~~max(x, 0), fy = ~~max(y, 0),
    fw = min(ccanvas.width - fx, w) + 0.5 | 0, fh = min(ccanvas.height - fy, h) + 0.5 | 0, cc;
...
Tproto.Weight = function (w, c, t, m, wmin, wmax, wnum) {
  ...
  if (m == 'colour') { this.colour = FindGradientColour(t, nweight, wnum); } else if (m == 'bgcolour') { this.bgColour = FindGradientColour(t, nweight, wnum); } else if (m == 'bgoutline') { this.bgOutline = FindGradientColour(t, nweight, wnum); } else if (m == 'outline') { this.outline.colour = FindGradientColour(t, nweight, wnum); } else if (m == 'size') {
...
Tproto.CheckActive = function (c, xoff, yoff) {
  ...
  return o.Active(c, t.mx, t.sp) ? o : null;
};
...
function TagCanvas(cid, lctr, opt) {
 ...
 this[i] = opt && Defined(opt[i]) ? opt[i]
  ...
  if (lctr && this.hideTags) {
    (function (t) {
      if (TagCanvas.loaded) { t.HideTags(); } else {
...
TCproto.Draw = function (t) {
  ...
  if (fixed && this.fixedAnim.active) {
    ...
    this.active = null;
    for (i = 0; i < l; ++i) {
      a = this.mx >= 0 && this.sp >= 0 && this.taglist[i].CheckActive(c, x, y);
...
TCproto.TooltipDiv = function (active, tag) {
  ...
  } else if (!active && this.mx != -1 && this.sp != -1 && this.ctitle.length) {
...
TCproto.AnimatePosition = function (w, h, t) {
  var tc = this, x = tc.mx, y = tc.sp, s, r;
...
TCproto.Clicked = function (e) {
  ...
  try {
    if (a && a.tag) {
 if (this.clickToFront === false || this.clickToFront === null) { a.tag.Clicked(e); } else {
