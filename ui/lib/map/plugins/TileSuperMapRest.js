...
export default class TileSuperMapRest extends TileImage {
  ...
  constructor(options) {
    ...
    const layerUrl = options.url + '/tileImage.' + options.format + '?';
    ...
    function tileUrlFunction(tileCoord, pixelRatio, projection) {
      ...
      const resolution = me.tileGrid.getResolution(z);
      const dpi = 96;
      ...
      const tileSize = toSize(me.tileGrid.getTileSize(z, me.tmpSize));
      const layerUrl = getFullRequestUrl.call(me);
  ...
  changeTilesVersion() {
    ...
    if (tileVersions && me.tempIndex < tileVersions.length && me.tempIndex >= 0) {
      const name = tileVersions[me.tempIndex].name;
      const result = me.mergeTileVersionParam(name);
      if (result) {
