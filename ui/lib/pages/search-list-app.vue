  ...
  <sp-wrapper title="搜索列表">
    ...
    <sp-card-list ref="list"
      ...
      <sp-panel slot-scope="{item}"
        ...
        <sp-description :width="40">
          <sp-avatar slot="title" theme="warning">{{item.name.substring(0,1)}}</sp-avatar>
          <sp-title :level="4" ellipsis :rows="1">{{item.title}}</sp-title>
          <sp-float>
            <sp-float-item>
              <sp-description top title="活跃用户">
                <sp-title :level="2">{{item.views}}</sp-title>
              </sp-description>
            </sp-float-item>
            <sp-float-item float="right">
              <sp-description top title="新增用户">
                <sp-title :level="2">{{item.views}}</sp-title>
              </sp-description>
            </sp-float-item>
          </sp-float>
        ...
        <template v-slot:action="{name, more}">
          ...
          <sp-icon v-else :name="name"></sp-icon>
        </template>
      </sp-panel>
    </sp-card-list>
  </sp-wrapper>
