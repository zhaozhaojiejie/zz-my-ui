  ...
  <sp-wrapper title="增删查改">
    ...
    <sp-crud ref="crud"
      ...
      <template v-slot:filter>
        <sp-input label="姓名" name="name"></sp-input>
        <sp-select label="性别" name="sex" :options="[{label:'男',value:1},{label:'女',value:0}]"></sp-select>
        <sp-input label="条件1" name="q1" collapsible></sp-input>
        <sp-input label="条件2" name="q2" collapsible></sp-input>
        <sp-input label="条件3" name="q3" collapsible></sp-input>
      </template>
      ...
      <template v-slot:form>
        <div>
          <sp-input label="姓名" name="name" :rules="{required:true}"></sp-input>
          <sp-date-picker label="出生日期" name="date" :props="{valueFormat:'yyyy-MM-dd'}"></sp-date-picker>
        </div>
      </template>

      <!-- 定义详情内容 -->
      <template v-slot:detail="{row}">
        <sp-detail border :column="2">
          <sp-detail-item label="姓名">{{row.name}}</sp-detail-item>
          <sp-detail-item label="身份证">{{row.id}}</sp-detail-item>
          <sp-detail-item label="出生日期">{{row.date}}</sp-detail-item>
          <sp-detail-item label="地区">{{row.county}}</sp-detail-item>
          <sp-detail-item label="简介">{{row.info}}</sp-detail-item>
        </sp-detail>
      </template>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        adapter: spCrud.adapter.call(this, 'MockForExample'),
        columns: [
          {
            type: 'selection'
          },
